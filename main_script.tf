...
resource "aws_subnet" "umami-subnet-public-1" {
...
resource "aws_subnet" "umami-subnet-private-1" {
  vpc_id                  = aws_vpc.umami-vpc.id
...
resource "aws_subnet" "umami-subnet-private-2" {
  vpc_id                  = aws_vpc.umami-vpc.id
...
resource "aws_route_table" "umami-public-crt" {
  vpc_id = aws_vpc.umami-vpc.id
...
resource "aws_route_table_association" "umami-crta-public-subnet-1" {
  subnet_id      = aws_subnet.umami-subnet-public-1.id
  route_table_id = aws_route_table.umami-public-crt.id
}
...
resource "aws_db_subnet_group" "RDS_subnet_grp" {
  subnet_ids = ["${aws_subnet.umami-subnet-private-1.id}", "${aws_subnet.umami-subnet-private-2.id}"]
}

# Create RDS instance
resource "aws_db_instance" "umamidb" {
...
data "template_file" "playbook" {
  ...
  vars = {
    ...
    db_RDS           = "${aws_db_instance.umamidb.endpoint}"
  }
}


# Create EC2 ( only after RDS is provisioned)
resource "aws_instance" "umamiec2" {
  ami             = var.ami
  ...
  depends_on = [aws_db_instance.umamidb]
}
...
# Play ansible playbook
  provisioner "local-exec" {
     command = "ANSIBLE_HOST_KEY_CHECKING=False ANSIBLE_DEPRECATION_WARNINGS=False ansible-playbook -u ubuntu -i ${aws_eip.eip.public_ip}, --private-key ${var.PRIV_KEY_PATH}  playbook-rendered.yml"
...
output "RDS-Endpoint" {
  value = aws_db_instance.umamidb.endpoint
}
